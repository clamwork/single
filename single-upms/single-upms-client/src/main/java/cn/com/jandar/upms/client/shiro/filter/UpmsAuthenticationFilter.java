package cn.com.jandar.upms.client.shiro.filter;

import cn.com.jandar.common.util.PropertiesFileUtil;
import cn.com.jandar.common.util.RedisUtil;
import cn.com.jandar.upms.client.shiro.session.UpmsSessionDao;
import cn.com.jandar.upms.client.util.RequestParameterUtil;
...
/**
 * 重写authc过滤器
 * Created by superman on 2017/3/11.
 */
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        // 判断请求类型
        String upmsType = PropertiesFileUtil.getInstance(UpmsConstant.JANDAR_UPMS_CLIENT).get("upms.type");
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        StringBuffer sso_server_url = new StringBuffer(PropertiesFileUtil.getInstance(UpmsConstant.JANDAR_UPMS_CLIENT).get("sso.server.url"));
        // server需要登录
        String upmsType = PropertiesFileUtil.getInstance(UpmsConstant.JANDAR_UPMS_CLIENT).get("upms.type");
        if ("server".equals(upmsType)) {
        ...
        sso_server_url.append("/sso/index").append("?").append("appid").append("=").append(PropertiesFileUtil.getInstance(UpmsConstant.JANDAR_UPMS_CLIENT).get("appID"));
        // 回跳地址
    ...
    private boolean validateClient(ServletRequest request, ServletResponse response) {
        ...
        // 判断局部会话是否登录
        String cacheClientSession = RedisUtil.get(JANDAR_UPMS_CLIENT_SESSION_ID + "_" + session.getId());
        if (StringUtils.isNotBlank(cacheClientSession)) {
            // 更新code有效期
            RedisUtil.set(JANDAR_UPMS_CLIENT_SESSION_ID + "_" + sessionId, cacheClientSession, timeOut);
            Jedis jedis = RedisUtil.getJedis();
            jedis.expire(JANDAR_UPMS_CLIENT_SESSION_IDS + "_" + cacheClientSession, timeOut);
            jedis.close();
            // 移除url中的code参数
            ...
            try {
                ...
                if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    if (1 == result.getIntValue("code") && result.getString("data").equals(code)) {
                        // code校验正确，创建局部会话
                        RedisUtil.set(JANDAR_UPMS_CLIENT_SESSION_ID + "_" + sessionId, code, timeOut);
                        // 保存code对应的局部会话sessionId，方便退出操作
                        RedisUtil.sadd(JANDAR_UPMS_CLIENT_SESSION_IDS + "_" + code, sessionId, timeOut);
                        _log.debug("当前code={}，对应的注册系统个数：{}个", code, RedisUtil.getJedis().scard(JANDAR_UPMS_CLIENT_SESSION_IDS + "_" + code));
                        // 移除url中的token参数
                        String backUrl = RequestParameterUtil.getParameterWithOutCode(WebUtils.toHttp(request));
                        // 返回请求资源
                        try {
